...
export const workflowRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: WorkflowCreateInput & { groupId?: string; folderId?: string } }>('/', async (request, reply) => {
    ...
    const workflow = await prisma.workflow.create({
      data: {
        ...
        pythonWorker: pythonCode.pythonWorker,
  ...
  app.post<{ 
    ...
    // Check admin permission for production promotion
    if (targetEnvironment === 'PD') {
      if (!user?.isAdmin) {
        return reply.status(403).send({ 
          error: 'Only administrators can promote workflows to Production (PD)' 
        });
      }
    }
